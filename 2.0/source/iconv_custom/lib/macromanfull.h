...
static const unsigned short macromanfull_2uni[128] = {
  /* 0x80 */
...
static int
macromanfull_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  if (c < 0x80)
    *pwc = (ucs4_t) c;
  else
    *pwc = (ucs4_t) macromanfull_2uni[c-0x80];
  return 1;
}

static const unsigned char macromanfull_page00[96] = {
...
static const unsigned char macromanfull_page01[104] = {
...
static const unsigned char macromanfull_page22[104] = {
...
static const unsigned char macromanfull_pagefb[8] = {
  0x00, 0xde, 0xdf, 0x00, 0x00, 0x00, 0x00, 0x00, /* 0x00-0x07 */
};

static int
macromanfull_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
  unsigned char c = 0;
  if (wc < 0x0080) {
    *r = wc;
    ...
    c = macromanfull_page00[wc-0x00a0];
  else if (wc >= 0x0130 && wc < 0x0198)
    c = macromanfull_page01[wc-0x0130];
  else if (wc >= 0x02c0 && wc < 0x02e0)
    c = macromanfull_page02[wc-0x02c0];
  else if (wc >= 0x03a8 && wc < 0x03c8)
    c = macromanfull_page03[wc-0x03a8];
  else if (wc >= 0x2010 && wc < 0x2048)
    c = macromanfull_page20[wc-0x2010];
  else if (wc == 0x20ac)
    c = 0xdb;
  else if (wc == 0x2122)
    c = 0xaa;
  else if (wc >= 0x2200 && wc < 0x2268)
    c = macromanfull_page22[wc-0x2200];
  else if (wc == 0x25ca)
    c = 0xd7;
  else if (wc == 0xf8ff)
    c = 0xf0;
  else if (wc >= 0xfb00 && wc < 0xfb08)
    c = macromanfull_pagefb[wc-0xfb00];
  if (c != 0) {
    *r = c;
    return 1;
  }
